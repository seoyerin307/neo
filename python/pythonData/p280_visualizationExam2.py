...
sizeSereis = mygrouping.size()

print('시리즈 3개를 이용하여 데이터 프레임 생성')
df = pd.concat([sumSeries, meanSeries, sizeSereis], axis=1)
df.columns = ['합계', '평균', '개수']
print(df)
print('-' * 50)
...
newgropuing = dftheater.groupby(['theater', 'region'])['bindo']
result = newgropuing.count()
print(result)
print('-' * 50)
